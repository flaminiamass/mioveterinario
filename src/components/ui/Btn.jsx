...
export default function Btn({ children, onClick, variant = "primary", small, style, disabled, loading }) {
  ...
  const base = { border: "none", borderRadius: radius.md, fontWeight: 600, cursor: isDisabled ? "not-allowed" : "pointer", opacity: isDisabled ? 0.5 : 1, padding: small ? "8px 14px" : "12px 18px", fontSize: small ? fontSize.md : fontSize.lg, minHeight: small ? 36 : 44, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6 };
  const variants = {
  ...
  return (
    <button disabled={isDisabled} onClick={onClick} className="btn-hover" style={{ ...base, ...variants[variant], ...style }}>
