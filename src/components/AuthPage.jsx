...
export default function AuthPage({ onNav }) {
  ...
  const getErrorMessage = (err) => {
    ...
    try { return JSON.stringify(err); } catch { return "Errore durante l'operazione."; }
  };
  ...
  const translateError = (err) => {
    ...
    if (msg.includes("Email not confirmed")) return "Devi confermare la tua email prima di accedere. Controlla la casella di posta.";
    ...
    if (msg.includes("duplicate key") || msg.includes("already exists")) return "Questa email è già registrata. Prova ad accedere.";
    return msg;
  };
  ...
  if (success) {
    return (
      <div style={{ maxWidth: 420, margin: "0 auto", padding: 24, textAlign: "center" }}>
        ...
        <div style={{ marginTop: 30, background: "#E8F5E9", borderRadius: radius.lg, padding: "20px", lineHeight: 1.7, color: "#2E7D32", fontSize: fontSize.base }}>
          <div style={{ fontSize: 36, marginBottom: 10 }}>✅</div>
          <b>Registrazione riuscita!</b>
          <p style={{ marginTop: 10 }}>{success}</p>
        </div>
        <Btn style={{ marginTop: 20, width: "100%" }} onClick={() => { setSuccess(null); setMode("login"); setPassword(""); }}>
          ← Torna al login
        </Btn>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ display: "flex", gap: 0, marginTop: 30, borderRadius: radius.lg, overflow: "hidden", border: `1px solid ${colors.border}` }}>
        <button
          onClick={() => { setMode("login"); setError(null); }}
          style={{
            flex: 1, padding: "12px", border: "none", cursor: "pointer", fontWeight: 700,
            fontSize: fontSize.base, fontFamily: "inherit",
      ...
      <form onSubmit={mode === "login" ? handleLogin : handleSignup} style={{ marginTop: 20 }}>
        ...
        {mode === "signup" && (
          <div style={{ marginBottom: 14 }}>
            <label htmlFor="auth-name" style={{ fontSize: fontSize.sm, fontWeight: 600, color: colors.textMuted, display: "block", marginBottom: 4 }}>
              Nome e cognome
            </label>
            <input
              id="auth-name"
        ...
        <div style={{ marginBottom: 14 }}>
          <label htmlFor="auth-email" style={{ fontSize: fontSize.sm, fontWeight: 600, color: colors.textMuted, display: "block", marginBottom: 4 }}>
            Email
          </label>
          <input
            id="auth-email"
        ...
        <div style={{ marginBottom: 14 }}>
          <label htmlFor="auth-password" style={{ fontSize: fontSize.sm, fontWeight: 600, color: colors.textMuted, display: "block", marginBottom: 4 }}>
            Password
          </label>
          <input
            id="auth-password"
        ...
        {mode === "signup" && (
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: fontSize.sm, fontWeight: 600, color: colors.textMuted, display: "block", marginBottom: 8 }}>
              Chi sei?
            </label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button
                ...
                style={{
                  padding: "14px 12px", borderRadius: radius.lg, cursor: "pointer",
              ...
              >
                ...
                <div style={{ fontWeight: 700, color: role === "owner" ? TEAL : colors.textMedium, fontSize: fontSize.base, marginTop: 4 }}>
                  Proprietario
                </div>
              ...
              >
                ...
                <div style={{ fontWeight: 700, color: role === "vet" ? ORANGE : colors.textMedium, fontSize: fontSize.base, marginTop: 4 }}>
                  Veterinario
                </div>
                <div style={{ fontSize: fontSize.sm, color: colors.textMuted, marginTop: 2 }}>
        ...
        {/* Errore */}
        {error && (
          <div style={{ background: colors.dangerBg, borderRadius: radius.md, padding: "10px 14px", marginBottom: 14, fontSize: fontSize.base, color: colors.dangerFg, lineHeight: 1.6 }}>
            ⚠️ {error}
          </div>
        )}
      ...
      {/* Disclaimer beta */}
      <div style={{ marginTop: 20, padding: "10px 16px", background: colors.bgOrangeLight, borderRadius: radius.lg, fontSize: fontSize.md, color: colors.textMedium, lineHeight: 1.6, textAlign: "center" }}>
