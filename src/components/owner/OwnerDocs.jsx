    ...
    <>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              fontWeight: 700,
              background: tab === k ? TEAL : colors.bgBtn,
              color: tab === k ? colors.white : colors.textMedium,
            }}
          >
            {l}
          </button>
      ...
      {tab === "referti" && (
        <div style={{ display: "grid", gap: 10 }}>
          ...
          >
            <b>ℹ️ I referti sono documenti clinici</b> redatti e firmati dal veterinario, che ne è l'unico responsabile.
            Non modificare o condividere il contenuto senza il consenso del professionista. Per farmaci soggetti a
            prescrizione fa fede la Ricetta Veterinaria Elettronica (REV) ufficiale.
          </div>
          {(() => {
            ...
            if (filtered.length === 0)
              return (
                <Card>
                  <Empty icon="📄" text="Nessun referto ancora" sub="Appariranno qui dopo le visite dal veterinario" />
                </Card>
              );
              ...
              return (
                <Card key={r.id} onClick={() => setOpen(isOpen ? null : r.id)}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontSize: fontSize.md, color: colors.textMuted }}>{r.date}</span>
                  </div>
                  ...
                  {isOpen && (
                    ...
                    >
                      <b style={{ color: TEAL }}>Diagnosi:</b> {r.diagnosis}
                      <br />
                      <b style={{ color: TEAL }}>Trattamenti:</b> {r.treatments}
                      <br />
                      <b style={{ color: TEAL }}>Farmaci:</b> {r.drugs}
                      <br />
                      <b style={{ color: TEAL }}>Indicazioni:</b> {r.advice}
                      <br />
      ...
      {tab === "fatture" && (
        <div style={{ display: "grid", gap: 10 }}>
          ...
          >
            <b>ℹ️ Le fatture</b> sono emesse dal veterinario/clinica che ha erogato la prestazione, che ne è il soggetto
            fiscalmente responsabile. Il documento generato da questa app è una bozza riepilogativa. Prima del go-live
            occorre integrare il flusso di fatturazione elettronica (SdI). [DA VALIDARE CON COMMERCIALISTA]
          </div>
          {(() => {
            ...
            if (filtered.length === 0)
              return (
                <Card>
                  <Empty icon="🧾" text="Nessuna fattura" sub="Le fatture emesse dal veterinario appariranno qui" />
                </Card>
              );
              ...
              return (
                <Card key={f.id} onClick={() => setOpen(isOpen ? null : f.id)}>
                  ...
                  <div style={{ fontSize: fontSize.md, color: colors.textSecondary }}>
                    ...
                    <span style={{ color: isPaid ? colors.success : colors.warning, fontWeight: 600 }}>
                    ...
                    <div style={{ marginTop: 10, borderTop: `1px solid ${colors.divider}`, paddingTop: 10 }}>
                      ...
                      <div style={{ marginTop: 8, fontSize: fontSize.md, color: colors.textSecondary }}>
                        {f.enpav > 0 && (
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <span>ENPAV 2%</span>
                            <span>€{f.enpav.toFixed(2)}</span>
                          </div>
                        )}
                        {f.iva > 0 && (
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <span>IVA 22%</span>
                            <span>€{f.iva.toFixed(2)}</span>
                          </div>
                        )}
                        {f.bollo > 0 && (
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <span>Bollo</span>
                            <span>€{f.bollo.toFixed(2)}</span>
                          </div>
                        )}
                      </div>
                      <Btn
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          const dest = {
                            fullName: f.destName || ownerProfile.fullName,
                            cf: f.destCf || ownerProfile.cf,
                            address: f.destAddress || ownerProfile.address,
                            email: f.destEmail || ownerProfile.email,
                            phone: f.destPhone || ownerProfile.phone,
                          };
