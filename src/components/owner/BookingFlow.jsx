...
export default function BookingFlow({
  ...
  const confirm = async () => {
    if (!visitReason.trim()) {
      notify("Indica il motivo della visita prima di confermare.");
      return;
    }
    const isAutoConfirm = vet.autoConfirm || false;
    const newStatus = isAutoConfirm ? "confirmed" : "pending";
    const intake = [
      visitReason ? `Motivo: ${visitReason}` : "",
      symptomDuration ? `Da quanto: ${symptomDuration}` : "",
      symptoms ? `Sintomi: ${symptoms}` : "",
      notes ? `Note: ${notes}` : "",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    if (isSupabaseConfigured() && user) {
      ...
      if (error) {
        notify("❌ Errore nel salvataggio: " + error.message);
        return;
      }
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
      <div
        style={{
          ...
          background: step >= n ? TEAL : colors.borderLight,
          color: step >= n ? colors.white : colors.textMuted,
          display: "flex",
          alignItems: "center",
  ...
  return (
    <>
      {step < 5 && (
        <Btn small variant="light" onClick={onCancel}>
          ← Annulla
        </Btn>
      )}
      <h2 style={{ margin: "14px 0 4px" }}>
        {step === 5
          ? vet.autoConfirm
            ...
            : "Richiesta inviata! ✅"
          : slotWasPreselected && step === 4
            ? "Conferma prenotazione"
            : `Prenota con ${vet.name}`}
      </h2>
      ...
      {!(slotWasPreselected && step === 4) && step < 5 && (
        <div style={{ display: "flex", margin: "14px 0 20px" }}>
          {stepDot(1, "Animale")}
          {stepDot(2, "Prestazione")}
          {stepDot(3, "Orario")}
          {stepDot(4, "Conferma")}
          {stepDot(5, "Fatto")}
        </div>
      ...
      {step === 1 && (
        <Card>
            ...
            {pets.map((p) => (
              <div
                ...
                style={{
                  padding: "10px 14px",
                  borderRadius: radius.lg,
                  border: `2px solid ${petId === p.id ? TEAL : colors.borderLight}`,
                  cursor: "pointer",
                  background: petId === p.id ? colors.bgTealSel : colors.white,
                }}
              >
                {p.photo} <b>{p.name}</b>{" "}
          ...
          <Btn onClick={() => setStep(2)} style={{ marginTop: 16, width: "100%" }} disabled={!petId}>
            Continua →
          </Btn>
        </Card>
      )}
      ...
      {step === 2 && (
        <Card>
            ...
            {availableCats.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      style={{ fontWeight: 700, fontSize: fontSize.xl, display: "flex", alignItems: "center", gap: 8 }}
                    >
                        ...
                        return (
                          ...
                          >
                            ...
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              ...
                              {selected && (
                                <div style={{ fontSize: fontSize.xs, color: TEAL, fontWeight: 600 }}>✓ Selezionato</div>
                              )}
                            </div>
                          </div>
                        );
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            <Btn variant="light" onClick={() => setStep(1)} style={{ flex: 1 }}>
              ← Indietro
            </Btn>
      ...
      {step === 3 && (
        <Card>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: radius.lg,
                    cursor: "pointer",
                    border: `2px solid ${date === d ? TEAL : colors.borderLight}`,
                    background: date === d ? colors.bgTealSel : colors.white,
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: `2px solid ${time === t ? TEAL : colors.borderLight}`,
                        background: time === t ? TEAL : taken ? colors.bgLight : colors.white,
                        color: time === t ? colors.white : taken ? colors.textMuted : "#333",
                        fontWeight: 600,
                        fontSize: fontSize.base,
                        minHeight: 44,
                        display: "flex",
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            <Btn variant="light" onClick={() => setStep(2)} style={{ flex: 1 }}>
              ← Indietro
            </Btn>
            <Btn onClick={() => setStep(4)} style={{ flex: 2 }} disabled={!date || !time}>
              Continua →
            </Btn>
          </div>
        </Card>
      ...
      {step === 4 && (
        <Card>
          <b style={{ fontSize: fontSize.xl }}>Riepilogo visita</b>
          {(() => {
            ...
            return (
              <div style={{ display: "grid", gap: 10, margin: "12px 0" }}>
                <div
                  style={{
                    background: "linear-gradient(135deg, #F0F9F9, #E0F2F2)",
                    borderRadius: radius.xl,
                    padding: 16,
                    border: `1px solid ${colors.borderLight}`,
                  }}
                >
                  <div style={{ fontSize: 28, fontWeight: 900, color: TEAL }}>
                    {time} <span style={{ fontSize: fontSize.base, color: colors.textMedium }}>· {date}</span>
                  </div>
                  <div style={{ marginTop: 8, fontSize: fontSize.base, color: colors.textDark, lineHeight: 1.7 }}>
                    🐾 <b>{p?.name}</b> ({p?.species})
                    <br />
                    {service?.emoji} <b>{service?.name}</b> · ~{service?.duration} min
                    <br />
                    👩‍⚕️ {vet.name} — {vet.clinic}
                    <br />
                    📍 {vet.address}
                  </div>
                  <div style={{ marginTop: 10, fontSize: 20, fontWeight: 900, color: ORANGE }}>€{service?.price}</div>
                </div>
                {slotWasPreselected && (
                  <button
                    onClick={() => setStep(1)}
                  ...
                  >
                    Modifica animale o prestazione
                  </button>
                )}
              </div>
            );
          ...
          {type === "video" && (
            ...
            >
              <b>⚠️ Video-consulto:</b> La consulenza a distanza non sostituisce una visita fisica quando clinicamente
              necessaria. Il veterinario valuterà se il video-consulto è appropriato per il tuo caso.
            </div>
          )}
          ...
          <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
            <label htmlFor="visit-reason" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              Motivo visita *
            </label>
            <input
              id="visit-reason"
              ...
              placeholder="Es. controllo, vaccino, prurito, zoppia…"
              style={inputStyle}
              maxLength={120}
            />
            <select value={symptomDuration} onChange={(e) => setSymptomDuration(e.target.value)} style={inputStyle}>
              <option value="">Da quanto tempo? (facoltativo)</option>
              <option value="Da oggi">Da oggi</option>
              <option value="Da qualche giorno">Da qualche giorno</option>
              <option value="Da più di una settimana">Da più di una settimana</option>
              <option value="Controllo periodico">Controllo periodico</option>
            </select>
            <textarea
              value={symptoms}
              onChange={(e) => setSymptoms(e.target.value)}
              placeholder="Sintomi principali (facoltativo)…"
              rows={2}
          ...
          <label
            ...
            style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, display: "block", marginTop: 10 }}
          >
            Note aggiuntive
          </label>
          <textarea
            ...
            placeholder="Note per il veterinario (facoltativo)…"
            rows={3}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
            <Btn variant="light" onClick={() => setStep(slotWasPreselected && serviceId ? 2 : 3)} style={{ flex: 1 }}>
              ← Indietro
            </Btn>
            <Btn variant="accent" onClick={confirm} style={{ flex: 2 }}>
              Conferma prenotazione ✓
            </Btn>
          </div>
        </Card>
      ...
      {step === 5 && (
        <Card style={{ textAlign: "center" }}>
          ...
          {vet.autoConfirm ? (
            ...
            >
              ✅ <b>Prenotazione confermata.</b> Segnati la data!
              <br />
            ...
            >
              ...
              <br />
              Il veterinario la confermerà o proporrà un'alternativa. Controlla "Visite" per gli aggiornamenti.
              <br />
              <span style={{ fontSize: fontSize.sm }}>📲 Riceverai un promemoria prima della visita.</span>
            </div>
          )}

          <Btn variant="accent" onClick={onDone} style={{ marginTop: 14, width: "100%" }}>
            Vai alle mie visite →
          </Btn>
        </Card>
      )}
