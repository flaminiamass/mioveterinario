...
export default function PetDetail({ pet, onBack }) {
  ...
  const [form, setForm] = useState({ name: pet.name, breed: pet.breed, weight: pet.weight || "", chip: pet.chip || "", sex: pet.sex || "" });
  ...
  return (
    <>
      <Btn small variant="light" onClick={handleBack}>← Indietro</Btn>
      <Card style={{ marginTop: 12, textAlign: "center" }}>
        ...
        <div style={{ color: colors.textSecondary, fontSize: fontSize.base }}>{pet.species} · {editing ? form.breed : pet.breed}</div>

        {!editing ? (
          <>
            <div style={{ display: "flex", justifyContent: "center", gap: 18, marginTop: 12, fontSize: fontSize.md, color: colors.textMedium, flexWrap: "wrap" }}>
          ...
          <div style={{ textAlign: "left", marginTop: 12 }}>
            <label htmlFor="pet-name" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600 }}>Nome</label>
            <input id="pet-name" style={inp} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />

            <label htmlFor="pet-breed" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, marginTop: 10, display: "block" }}>Razza</label>
            <input id="pet-breed" style={inp} value={form.breed} onChange={e => setForm({ ...form, breed: e.target.value })} />

            <label htmlFor="pet-weight" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, marginTop: 10, display: "block" }}>Peso (kg)</label>
            <input id="pet-weight" style={inp} type="number" value={form.weight} onChange={e => setForm({ ...form, weight: e.target.value })} />

            <label htmlFor="pet-chip" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, marginTop: 10, display: "block" }}>Microchip</label>
            {/* Avviso microchip — MEDIUM fix: il numero è identificativo dell'animale e può essere associato all'anagrafe */}
            <p style={{ fontSize: fontSize.xs, color: colors.textMuted, margin: "2px 0 0" }}>Il numero di microchip è un dato identificativo dell'animale. Viene mostrato parzialmente nelle visualizzazioni pubbliche.</p>
            <input id="pet-chip" style={inp} value={form.chip} onChange={e => setForm({ ...form, chip: e.target.value.replace(/\D/g, "").slice(0, 15) })} placeholder="15 cifre (es: 380260101234567)" maxLength={15} />

            <label htmlFor="pet-sex" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, marginTop: 10, display: "block" }}>Sesso</label>
            <select id="pet-sex" style={{ ...inp, background: colors.white }} value={form.sex} onChange={e => setForm({ ...form, sex: e.target.value })}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              <Btn variant="light" onClick={() => { setForm({ ...savedForm.current }); setEditing(false); }} style={{ flex: 1 }}>Annulla</Btn>
              <Btn onClick={saveEdit} style={{ flex: 1 }} disabled={!form.name}>Salva ✓</Btn>
            </div>
          </div>
        )}
      </Card>
      ...
      {/* Vaccini */}
      <SectionTitle style={{ marginTop: 18 }} right={<Btn small variant="accent" onClick={() => setAddingVax(!addingVax)}>{addingVax ? "Annulla" : "+ Vaccino"}</Btn>}>💉 Libretto vaccini</SectionTitle>

      {addingVax && (
        <Card style={{ marginBottom: 10 }}>
          <label htmlFor="vax-name" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600 }}>Nome vaccino *</label>
          <input id="vax-name" style={vaxInp} placeholder="Es: Polivalente, Rabbia…" value={vaxForm.name} onChange={e => setVaxForm({ ...vaxForm, name: e.target.value })} />

          <label htmlFor="vax-date" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, marginTop: 10, display: "block" }}>Data somministrazione *</label>
          <input id="vax-date" style={vaxInp} type="date" value={vaxForm.date} onChange={e => setVaxForm({ ...vaxForm, date: e.target.value })} />

          <label htmlFor="vax-due" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600, marginTop: 10, display: "block" }}>Scadenza richiamo</label>
          <input id="vax-due" style={vaxInp} type="date" value={vaxForm.due} onChange={e => setVaxForm({ ...vaxForm, due: e.target.value })} />
      ...
      {petAppts.length ? petAppts.map(a => {
        const v = vets.find(x => x.id === a.vetId);
        const ref = referti.find(r => r.apptId === a.id);
        return (
          <Card key={a.id} style={{ marginBottom: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <b style={{ fontSize: fontSize.base }}>{a.date} · {a.time}</b><Badge status={a.status} />
            </div>
            <div style={{ fontSize: fontSize.md, color: colors.textSecondary, marginTop: 4 }}>{v?.name} · {a.serviceId ? `${getService(a.serviceId)?.emoji || ""} ${getService(a.serviceId)?.name || ""}` : TYPE_META[a.type]}</div>
            {a.vetNotes && <div style={{ fontSize: fontSize.md, color: TEAL, marginTop: 6, background: colors.bgTealSel, borderRadius: radius.sm, padding: "6px 10px" }}>👩‍⚕️ Note vet: {a.vetNotes}</div>}
            {ref && <div style={{ marginTop: 6, fontSize: fontSize.md, background: colors.bgTealSel, borderRadius: radius.sm, padding: "6px 10px" }}>📄 Referto: <b>{ref.title}</b></div>}
          </Card>
        );
