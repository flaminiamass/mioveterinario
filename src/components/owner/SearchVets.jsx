...
const P2B_TEXT = {
  ...
  recommended: "ℹ️ Ordine per combinazione di valutazione, numero di recensioni e disponibilità prossima. Nessun risultato sponsorizzato.",
};
...
export default function SearchVets({ onView, onBookService, initialMode }) {
  ...
  const filtered = useMemo(() => {
    ...
    let r = verified.filter(v => {
      ...
      if (q) {
        ...
        const match = v.name.toLowerCase().includes(lc)
    ...
    if (sort === "rating") withSlot.sort((a, b) => b.rating - a.rating);
      ...
      withSlot.sort((a, b) => {
        const scoreA = (a.rating / 5) * 0.4
          + ((a.reviews || 0) / maxReviews) * 0.3
          + (a._firstSlot ? 0.3 : 0);
        const scoreB = (b.rating / 5) * 0.4
          + ((b.reviews || 0) / maxReviews) * 0.3
          + (b._firstSlot ? 0.3 : 0);
  ...
  if (mode === "service") {
    return (
      <>
        {/* Sub-tab switcher */}
        <div style={{ display: "flex", gap: 0, marginBottom: 14, borderRadius: radius.lg, overflow: "hidden", border: `1px solid ${colors.borderLight}` }}>
          <button onClick={() => setMode("vet")} style={{ flex: 1, padding: "10px 0", border: "none", cursor: "pointer", fontWeight: 600, fontSize: fontSize.base, fontFamily: "inherit", background: colors.white, color: colors.textMedium }}>🔍 Per veterinario</button>
          <button style={{ flex: 1, padding: "10px 0", border: "none", cursor: "default", fontWeight: 700, fontSize: fontSize.base, fontFamily: "inherit", background: TEAL, color: colors.white }}>💊 Per servizio</button>
        </div>
  ...
  return (
    <>
      {/* Sub-tab switcher */}
      <div style={{ display: "flex", gap: 0, marginBottom: 14, borderRadius: radius.lg, overflow: "hidden", border: `1px solid ${colors.borderLight}` }}>
        <button style={{ flex: 1, padding: "10px 0", border: "none", cursor: "default", fontWeight: 700, fontSize: fontSize.base, fontFamily: "inherit", background: TEAL, color: colors.white }}>🔍 Per veterinario</button>
        <button onClick={() => setMode("service")} style={{ flex: 1, padding: "10px 0", border: "none", cursor: "pointer", fontWeight: 600, fontSize: fontSize.base, fontFamily: "inherit", background: colors.white, color: colors.textMedium }}>💊 Per servizio</button>
      </div>

      {/* Barra ricerca */}
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Nome, specializzazione, città o zona…" style={searchInputStyle} />
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
        {[
          ...
          <button key={key} onClick={() => setQuickDate(quickDate === key ? "" : key)} style={{
            padding: "7px 12px", minHeight: 36, borderRadius: radius.pill, border: "none",
            cursor: "pointer", fontSize: fontSize.md, fontWeight: 600,
      ...
      <button onClick={() => setShowFilters(!showFilters)} style={{
        background: "none", border: "none", cursor: "pointer", fontSize: fontSize.md,
        color: TEAL, fontWeight: 600, padding: "4px 0", marginTop: 2,
      }}>
        ...
        <div style={{ marginTop: 6 }}>
          ...
          {allSpecs.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: fontSize.sm, color: colors.textMuted, marginBottom: 4, fontWeight: 600 }}>Specializzazione</div>
              <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, WebkitOverflowScrolling: "touch" }}>
                {allSpecs.map(s => {
                  const on = specs.includes(s);
                  return (
                    <button key={s} onClick={() => setSpecs(on ? specs.filter(x => x !== s) : [...specs, s])} style={{
                      padding: "5px 10px", borderRadius: radius.pill, border: "none", cursor: "pointer",
                      fontSize: fontSize.sm, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0,
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <input type="number" value={priceMin} onChange={e => setPriceMin(e.target.value)} placeholder="Da €" style={{ ...compactInputStyle, width: 80 }} min={0} />
            <span style={{ color: colors.textMuted }}>—</span>
            <input type="number" value={priceMax} onChange={e => setPriceMax(e.target.value)} placeholder="A €" style={{ ...compactInputStyle, width: 80 }} min={0} />
          </div>
        </div>
      )}
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: fontSize.xl }}>
                  {v.name}{" "}
                  <span style={{ fontSize: fontSize.xs, color: TEAL, fontWeight: 600, background: colors.bgTealLight, padding: "2px 7px", borderRadius: radius.md }}>✓ Verificato</span>
                </div>
                ...
                <div style={{ marginTop: 4 }}>
                  <Stars n={v.rating} />{" "}
                  <span style={{ fontSize: fontSize.md, color: colors.textSecondary }}>{v.rating} ({v.reviews} recensioni)</span>
                </div>
                ...
                <div style={{ marginTop: 6, display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {v.spec.map(s => (
                    <span key={s} style={{
                      ...
                      fontSize: fontSize.xs, padding: "3px 8px", borderRadius: radius.md, fontWeight: 600,
                    }}>{s}</span>
                  ))}
                </div>
                ...
                <div style={{ marginTop: 4, display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {v.types.map(t => (
                    <span key={t} style={{
                      background: colors.bgOrangeLight, color: ORANGE,
                      fontSize: fontSize.xs, padding: "2px 7px", borderRadius: radius.sm, fontWeight: 600,
                    }}>{TYPE_META[t]}</span>
                  ))}
                </div>
              </div>
