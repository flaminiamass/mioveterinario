  ...
  return (
    <>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <Btn small variant="light" onClick={() => setWeekOffset(weekOffset - 1)}>
          {isMobile ? "←" : "← Sett. prec."}
        </Btn>
        <b style={{ alignSelf: "center", fontSize: fontSize.base, textAlign: "center" }}>
          {weekOffset === 0 ? "Questa settimana" : `${fmtDate(weekDays[0])} → ${fmtDate(weekDays[5])}`}
        </b>
        <Btn small variant="light" onClick={() => setWeekOffset(weekOffset + 1)}>
          {isMobile ? "→" : "Succ. →"}
        </Btn>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {weekDays.map((d) => {
          ...
          const dayAppts = appts
            .filter((a) => a.vetId === vetId && a.date === ds && a.status !== "cancelled")
          ...
          return (
            <Card key={ds} style={{ borderLeft: isToday ? `4px solid ${ORANGE}` : "4px solid transparent" }}>
              <b style={{ fontSize: fontSize.base, color: isToday ? ORANGE : colors.textDark }}>
                {(isMobile ? DAY_SHORT : DAY_FULL)[d.getDay()]} {d.getDate()}/{d.getMonth() + 1}
                {isToday ? " · OGGI" : ""}
              </b>
              {dayAppts.length ? (
                dayAppts.map((a) => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <b style={{ fontSize: fontSize.base }}>{a.time}</b> · {pet?.photo} {pet?.name}{" "}
                        <span style={{ color: colors.textSecondary, fontSize: fontSize.md }}>({pet?.species})</span>
