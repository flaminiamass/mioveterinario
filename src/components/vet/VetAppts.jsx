            ...
            <Card key={a.id}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
                  ...
                  {svc && <div style={{ fontSize: fontSize.md, color: ORANGE, fontWeight: 600, marginTop: 2 }}>€{svc.price}</div>}
              ...
              {a.proposal && a.proposal.from === "owner" && (
                <div style={{ marginTop: 10, background: colors.bgOrangeLight, borderRadius: radius.md, padding: "10px 12px" }}>
                  <div style={{ fontWeight: 700, fontSize: fontSize.base, color: ORANGE }}>📝 Il proprietario propone una modifica:</div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                {a.status === "pending" && <>
                  <Btn small onClick={() => setStatus(a.id, "confirmed")}>✓ Conferma</Btn>
                  <Btn small variant="danger" onClick={() => setRejectingId(a.id)}>✗ Rifiuta</Btn>
                  {!a.proposal && <Btn small variant="ghost" onClick={() => { setProposingId(a.id); setPropDate(""); setPropTime(""); setPropMsg(""); }}>📅 Proponi alternativa</Btn>}
                </>}
                {a.status === "confirmed" && <>
                  <Btn small variant="light" onClick={() => setStatus(a.id, "completed")}>Segna completata</Btn>
                  {!a.vetNotes && <Btn small variant="ghost" onClick={() => { setNotingId(a.id); setNoteText(""); }}>📝 Nota</Btn>}
                  {!a.proposal && <Btn small variant="ghost" onClick={() => { setProposingId(a.id); setPropDate(""); setPropTime(""); setPropMsg(""); }}>📅 Proponi cambio</Btn>}
                </>}
                {a.status === "completed" && !hasRef && <Btn small variant="accent" onClick={() => setRefertoFor(a)}>+ Referto</Btn>}
                {a.status === "completed" && !hasInv && <Btn small variant="ghost" onClick={() => setInvoiceFor(a)}>+ Fattura</Btn>}
                {hasRef && <span style={{ fontSize: fontSize.sm, color: colors.success, fontWeight: 600, alignSelf: "center" }}>📄 Referto ✓</span>}
                {hasInv && <span style={{ fontSize: fontSize.sm, color: colors.success, fontWeight: 600, alignSelf: "center" }}>🧾 Fattura ✓</span>}
              </div>

              {/* Inline: aggiungi nota vet */}
              {notingId === a.id && (
                <div style={{ marginTop: 10, borderTop: `1px solid ${colors.divider}`, paddingTop: 10 }}>
                  <textarea id={`note-${a.id}`} value={noteText} onChange={e => setNoteText(e.target.value)} rows={2}
              ...
              {proposingId === a.id && (
                <div style={{ marginTop: 10, borderTop: `1px solid ${colors.divider}`, paddingTop: 10 }}>
                  ...
                  <textarea value={propMsg} onChange={e => setPropMsg(e.target.value)} placeholder="Messaggio per il proprietario (facoltativo)…" rows={2}
                    style={{ ...inputStyle, borderRadius: radius.md, marginTop: 8 }} />
                  <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <Btn small variant="light" onClick={() => setProposingId(null)}>Annulla</Btn>
