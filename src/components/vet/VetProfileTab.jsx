...
export default function VetProfileTab({ vetId }) {
  ...
  const [customServiceSeq, setCustomServiceSeq] = useState(0);
  ...
  const addCustomService = async () => {
    if (db.isSupabaseConfigured()) {
      ...
      const newSvc = {
        ...
        price: Number(data.custom_price),
        duration: Number(data.custom_duration),
        cat: data.custom_category,
        emoji: data.custom_emoji,
        desc: data.custom_desc || "",
      };
      setVets(vets.map((v) => (v.id === vetId ? { ...v, services: [...vetServices, newSvc] } : v)));
    } else {
      const nextSeq = Math.max(customServiceSeq, customServices.length) + 1;
      setCustomServiceSeq(nextSeq);
      const newSvc = {
        id: `c_${vetId}_${nextSeq}`,
        name: customForm.name,
    ...
    <>
      <Card style={{ textAlign: "center" }}>
        ...
        <div style={{ margin: "8px 0" }}>
          <Stars n={vet.rating} /> <b>{vet.rating}</b> · {vet.reviews} recensioni
        </div>
          ...
          {ALL_DAYS.map((d) => {
            ...
            return (
              <button
                ...
                aria-label={`${d.label}: ${active ? "attivo" : "disattivo"}`}
                style={{
                  padding: "8px 14px",
                  borderRadius: radius.md,
                  border: `2px solid ${active ? TEAL : colors.borderLight}`,
      ...
      <Card style={{ marginTop: 10 }}>
        <div style={{ fontWeight: 800, color: colors.textDark, marginBottom: 10 }}>⚙️ Regole disponibilità demo</div>
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <label style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              Mattina start
              <input
                style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                value={availabilityDraft.morningStart}
                onChange={(e) => setAvailabilityDraft({ ...availabilityDraft, morningStart: e.target.value })}
              />
            </label>
            <label style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              Mattina end
              <input
                style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                value={availabilityDraft.morningEnd}
                onChange={(e) => setAvailabilityDraft({ ...availabilityDraft, morningEnd: e.target.value })}
              />
            </label>
            <label style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              Pomeriggio start
              <input
                style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                value={availabilityDraft.afternoonStart}
                onChange={(e) => setAvailabilityDraft({ ...availabilityDraft, afternoonStart: e.target.value })}
              />
            </label>
            <label style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              Pomeriggio end
              <input
                style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                value={availabilityDraft.afternoonEnd}
                onChange={(e) => setAvailabilityDraft({ ...availabilityDraft, afternoonEnd: e.target.value })}
              />
            </label>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <label style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              ...
              <input
                ...
                min="0"
                style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                value={availabilityDraft.buffer}
                onChange={(e) => setAvailabilityDraft({ ...availabilityDraft, buffer: Number(e.target.value) })}
              />
            </label>
            <label style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 700 }}>
              Anticipo minimo (min)
              <input
                ...
                min="0"
                style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                value={availabilityDraft.minLead}
                onChange={(e) => setAvailabilityDraft({ ...availabilityDraft, minLead: Number(e.target.value) })}
              />
            </label>
          </div>
          <button
            ...
            style={{
              minHeight: 44,
              border: `2px solid ${vet.autoConfirm ? TEAL : colors.borderLight}`,
              borderRadius: radius.lg,
              background: vet.autoConfirm ? colors.bgTealSel : colors.white,
              color: vet.autoConfirm ? TEAL : colors.textMedium,
              fontWeight: 800,
              fontFamily: "inherit",
              cursor: "pointer",
            }}
          >
            {vet.autoConfirm ? "⚡ Auto-confirm ON" : "⏳ Auto-confirm OFF"}
          </button>
          <div style={{ fontSize: fontSize.sm, color: colors.textMuted, lineHeight: 1.5 }}>
            Demo UI: questi campi preparano il futuro calendario reale. Per ora salvano solo lo stato visivo locale.
          </div>
        </div>
      </Card>
          ...
          <Card key={cat} style={{ marginBottom: 8, padding: 0, overflow: "hidden" }}>
            ...
            >
              <span style={{ fontWeight: 700, fontSize: fontSize.base }}>
                {CAT_EMOJI[cat]} {cat}{" "}
                <span style={{ color: colors.textMuted, fontWeight: 400, fontSize: fontSize.md }}>
                  ({activeInCat}/{catServices.length} attivi)
                </span>
              </span>
              <span
                style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        aria-label={`${isActive ? "Disattiva" : "Attiva"} ${svc.name}`}
                        style={{
                          width: 36,
                          height: 22,
                          borderRadius: 11,
                          border: "none",
                          cursor: "pointer",
                          background: isActive ? TEAL : colors.borderLight,
                          position: "relative",
                          flexShrink: 0,
                      ...
                      {isActive ? (
                        ...
                        <span style={{ fontSize: fontSize.md, color: colors.textMuted, flexShrink: 0 }}>
                          €{svc.price}
                        </span>
                      )}
                    </div>
            ...
            <Card key={svc.id} style={{ marginBottom: 8 }}>
              {editingCustom === svc.id ? (
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <Btn small onClick={() => saveEditCustom(svc.id)} disabled={!editForm.name || !editForm.price}>
                      Salva ✓
                    </Btn>
                    <Btn small variant="light" onClick={() => setEditingCustom(null)}>
                      Annulla
                    </Btn>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: fontSize.sm, color: colors.textMuted }}>
                      ⏱ ~{svc.duration} min · {svc.cat}
                    </div>
                  </div>
                  <b style={{ color: ORANGE, fontSize: fontSize.xl }}>€{svc.price}</b>
                  <div style={{ display: "flex", gap: 4 }}>
                    <Btn small variant="ghost" onClick={() => startEditCustom(svc)}>
                      ✏️
                    </Btn>
                    <Btn small variant="ghost" onClick={() => removeCustom(svc.id)}>
                      🗑️
                    </Btn>
                  </div>
                </div>
      ...
      {addingCustom ? (
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <select
                  ...
                  onChange={(e) => setCustomForm({ ...customForm, cat: e.target.value })}
                >
                  {SERVICE_CATEGORIES.map((c) => (
                    <option key={c}>{c}</option>
                  ))}
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
              <Btn
                ...
                disabled={!customForm.name || !customForm.price || !customForm.duration}
                style={{ flex: 1 }}
              >
                Aggiungi ✓
              </Btn>
              <Btn variant="light" onClick={() => setAddingCustom(false)} style={{ flex: 1 }}>
                Annulla
              </Btn>
            </div>
          </div>
        ...
        <Btn variant="accent" onClick={() => setAddingCustom(true)} style={{ marginTop: 10, width: "100%" }}>
        ...
        {mine.map((r) => (
          <Card key={r.id}>
            ...
            {r.reply ? (
              ...
              >
                <b style={{ color: TEAL }}>La tua risposta:</b> {r.reply}
              </div>
            ) : replyFor === r.id ? (
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                  ...
                  <Btn small variant="light" onClick={() => setReplyFor(null)}>
                    Annulla
                  </Btn>
                </div>
              </div>
            ) : (
              <Btn small variant="ghost" style={{ marginTop: 8 }} onClick={() => setReplyFor(r.id)}>
                ↩ Rispondi
              </Btn>
            )}
          </Card>
