...
export default function InvoiceForm({ appt, vetId, onDone }) {
  ...
  return (
    <>
      <Btn small variant="light" onClick={onDone}>
        ← Indietro
      </Btn>
      <Card style={{ marginTop: 12 }}>
        <h3 style={{ marginTop: 0 }}>
          Nuova fattura · N° {invoiceNumber}{" "}
          <span style={{ fontSize: fontSize.sm, color: colors.warning, fontWeight: 600 }}>
            [BOZZA — non è fattura elettronica]
          </span>
        </h3>
        ...
        >
          <b>⚠️ Attenzione fiscale:</b> Questo documento è una bozza riepilogativa generata dalla piattaforma. Non
          costituisce fattura elettronica ai sensi della normativa italiana (D.Lgs. 127/2015). Per la fattura
          fiscalmente valida è necessario il flusso SdI/Agenzia Entrate. I calcoli di ENPAV e IVA sono orientativi:
          verificarli con il proprio commercialista. [DA VALIDARE CON COMMERCIALISTA]
        </div>

        {/* Selezione cliente */}
        {clients.length > 0 && (
          <div style={{ marginBottom: 14 }}>
            <label htmlFor="inv-client" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600 }}>
              Cliente
            </label>
            <select
              id="inv-client"
        ...
        >
          ...
          <div>
            ...
            <details style={{ marginTop: 6 }}>
              <summary style={{ fontSize: fontSize.sm, color: TEAL, cursor: "pointer", fontWeight: 600 }}>
                ✏️ Modifica dati
              </summary>
              <div style={{ display: "grid", gap: 6, marginTop: 6 }}>
                <input
                  ...
                  value={dest.fullName}
                  onChange={(e) => setDest({ ...dest, fullName: e.target.value })}
                  placeholder="Nome completo"
                />
                <input
                  style={compactInputStyle}
                  value={dest.cf}
                  onChange={(e) => setDest({ ...dest, cf: e.target.value.toUpperCase() })}
        ...
        {items.map((it, ix) => (
          <div
            ...
            style={{
              display: "grid",
              gridTemplateColumns: isMobile ? "1fr" : "2fr 80px 100px 30px",
              gap: 6,
              marginBottom: 8,
              alignItems: "center",
            }}
          >
            <input
              ...
              value={it.desc}
              onChange={(e) => upd(ix, "desc", e.target.value)}
            />
            <div style={{ display: isMobile ? "flex" : "contents", gap: 6 }}>
              <input
                ...
                placeholder="Qtà"
                value={it.qty}
                onChange={(e) => upd(ix, "qty", e.target.value)}
              />
              <input
                style={{ ...compactInputStyle, flex: 1 }}
                type="number"
                ...
                placeholder="€"
                value={it.price}
                onChange={(e) => upd(ix, "price", e.target.value)}
              />
            </div>
            {items.length > 1 && (
              <button
        ...
        <Btn small variant="light" onClick={() => setItems([...items, { desc: "", qty: 1, price: 0 }])}>
          + Aggiungi voce
        </Btn>

        <div style={{ marginTop: 14 }}>
          <label htmlFor="inv-pay" style={{ fontSize: fontSize.sm, color: colors.textMuted, fontWeight: 600 }}>
            Pagamento
          </label>
          <select
            id="inv-pay"
        ...
        <div style={{ marginTop: 16, borderTop: `2px solid ${colors.divider}`, paddingTop: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: fontSize.base, marginBottom: 4 }}>
          ...
          {!isForfe && (
            <>
              ...
              >
                <span>Rivalsa ENPAV 2%</span>
                <span>€{enpav.toFixed(2)}</span>
              </div>
              <div
                style={{
                  display: "flex",
        ...
        <Btn variant="accent" style={{ marginTop: 14, width: "100%" }} disabled={imponibile <= 0} onClick={emitInvoice}>
          Salva bozza fattura
        </Btn>
      </Card>
    </>
