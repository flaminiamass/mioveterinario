  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[["Fatturato", `€${total.toFixed(0)}`, TEAL], ["Incassato", `€${paid.toFixed(0)}`, colors.success], ["Fatture", mine.length, ORANGE]].map(([l, v, c]) => (
          ...
          return (
            <Card key={f.id}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <b>🧾 {f.number || f.id.toUpperCase()} · {f.date}</b>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, alignItems: "center", gap: 6 }}>
                <span style={{ fontSize: fontSize.md, fontWeight: 700, color: f.status === "paid" ? colors.success : colors.warning }}>{f.status === "paid" ? "✓ Pagata" : "Da pagare"}</span>
                <div style={{ display: "flex", gap: 6 }}>
                  <Btn small variant="light" onClick={() => {
