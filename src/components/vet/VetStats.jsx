...
export default function VetStats({ vetId }) {
  ...
  const monthAppts = appts.filter(a =>
    a.vetId === vetId && a.status === "completed" && a.date >= monthStart && a.date < monthEnd
  );

  /* Incassi del mese (fatture pagate) */
  const monthInvoices = invoices.filter(f =>
    f.vetId === vetId && f.status === "paid" && f.date >= monthStart && f.date < monthEnd
  );
  const monthRevenue = monthInvoices.reduce((sum, f) => sum + (f.total || 0), 0);

  /* Top 3 servizi più richiesti (tutti i tempi) */
  const serviceCounts = {};
  appts.filter(a => a.vetId === vetId && a.status === "completed" && a.serviceId).forEach(a => {
      ...
      {topServices.length > 0 && (
        <Card style={{ padding: 14 }}>
          ...
          {topServices.map(({ service, count }, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: i < topServices.length - 1 ? `1px solid ${colors.divider}` : "none" }}>
              <span style={{ fontSize: 20 }}>{service?.emoji || "🩺"}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: fontSize.base }}>{service?.name || "Servizio"}</div>
              </div>
              <div style={{ fontWeight: 700, color: TEAL, fontSize: fontSize.base }}>{count} {count === 1 ? "visita" : "visite"}</div>
            </div>
          ))}
        </Card>
      )}
