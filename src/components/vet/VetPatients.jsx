...
export default function VetPatients({ vetId }) {
  ...
  const myClientIds = [
    ...new Set(clients.filter((cl) => cl.petIds.some((pid) => myPetIds.includes(pid))).map((cl) => cl.id)),
  ];
  ...
  const filteredClients = myClients.filter((cl) => {
    ...
    return (
      cl.fullName.toLowerCase().includes(lower) ||
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {filteredClients.length === 0 && myClients.length === 0 && (
          <Empty icon="🐾" text="Non hai ancora pazienti" sub="Appariranno qui quando riceverai prenotazioni" />
        )}
        {filteredClients.length === 0 && myClients.length > 0 && (
          <Empty icon="🔍" text="Nessun cliente trovato" sub="Prova a cambiare la ricerca" />
        )}
                  ...
                  {editingClient === cl.id ? (
                    <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        <div>
                          ...
                          <input
                            style={{ ...compactInputStyle, width: "100%", marginTop: 4 }}
                            value={editForm.cf}
                            onChange={(e) => setEditForm({ ...editForm, cf: e.target.value.toUpperCase() })}
                            maxLength={16}
                          />
                        </div>
                        <div>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <Btn small onClick={() => saveEdit(cl.id)} disabled={!editForm.fullName}>
                          Salva ✓
                        </Btn>
                        <Btn small variant="light" onClick={() => setEditingClient(null)}>
                          Annulla
                        </Btn>
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      <Btn small variant="ghost" style={{ marginTop: 8 }} onClick={() => startEdit(cl)}>
                        ✏️ Modifica dati
                      </Btn>
                    </div>
                  )}
                      ...
                      >
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: fontSize.sm, color: colors.textMuted }}>
                              {visits.length} visite · {refs.length} referti
                            </div>
                          </div>
                        </div>
                        {refs.length > 0 && (
