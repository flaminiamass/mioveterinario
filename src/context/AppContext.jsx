...
export function AppProvider({ children }) {
  ...
  async function loadOwnerData() {
    ...
    if (vetsData && vetsData.length > 0) {
      ...
      const { data: allVetServices } = await supabase
        .from("vet_services")
        .select("*")
        .in("vet_id", vetIds);
  ...
  async function loadVetData() {
    ...
    const { data: vetServicesData } = await supabase
      .from("vet_services")
      .select("*")
      .eq("vet_id", vetRow.id);
    ...
    if (ownerIds.length > 0) {
      ...
      const { data: clientPetsData } = await supabase
        .from("pets")
        .select("id, owner_id")
        .in("id", petOwnerIds);
      ...
      if (petOwnerIds.length > 0) {
        const { data: allClientPets } = await supabase
          .from("pets")
          .select("*")
          .in("id", petOwnerIds);
